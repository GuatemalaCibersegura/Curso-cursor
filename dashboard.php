...
try {
    ...
    if ($conn) {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) as total, COALESCE(SUM(cost), 0) as income FROM services WHERE service_date = CURDATE() AND status = 'completed'");
        $stmt->execute();
        $result = $stmt->fetch();
        ...
        $stmt = $conn->prepare("SELECT COALESCE(SUM(cost), 0) as income FROM services WHERE YEAR(service_date) = YEAR(CURDATE()) AND MONTH(service_date) = MONTH(CURDATE()) AND status = 'completed'");
        $stmt->execute();
        $result = $stmt->fetch();
